...
const PrismBackground = () => {
                ...
                if(t < MAX_DIST) {
                    ...
                    if(length(refractDir) > 0.0) {
                        ...
                        if(t2 < MAX_DIST) {
                            ...
                            // Lime green theme colors (#D1E64F variations)
                            vec3 limeBase = vec3(0.82, 0.90, 0.31);
                            vec3 limeDark = vec3(0.6, 0.7, 0.2);
                            vec3 limeLight = vec3(0.9, 0.95, 0.5);
                            
                            vec3 bgR = getBackground(r) * limeBase * 1.5;
                            vec3 bgG = getBackground(g) * limeLight * 1.2;
                            vec3 bgB = getBackground(b) * limeDark * 1.8;
                            
                            color = vec3(bgR.x, bgG.y, bgB.z);
                            color = pow(color, vec3(0.7)) * 4.0;
                    ...
                    color += spec * vec3(0.9, 0.95, 0.5) * 3.0;
                    
                    // Lime green fresnel
                    vec3 limeFresnel = vec3(0.82, 0.90, 0.31);
                    vec3 fresnelColor = limeFresnel * (0.5 + 0.5 * sin(fresnel * TAU + uTime));
                    color += fresnel * fresnelColor * 1.5;
                    
                    float edge = pow(1.0 - abs(dot(viewDir, normal)), 4.0);
                    color += edge * vec3(0.7, 0.8, 0.3) * 0.8;
                    
                    float sss = pow(max(dot(-normal, lightDir), 0.0), 2.0);
                    color += sss * vec3(0.8, 0.9, 0.4) * 0.6;
                    
                }
                
                gl_FragColor = vec4(color, t < MAX_DIST ? 1.0 : 0.0);
            }
        `;
    ...
    return (
        <div className="prism-background">
            <canvas ref={canvasRef} id="prism-canvas"></canvas>
        </div>
    );
};
